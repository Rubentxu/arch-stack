...
use crate::code::class_diagram::{
    ClassDiagramReport, ClassEdgeKind, ClassNode, Language as CdLanguage, TypeKind,
};
...
pub fn print_human_table(report: &DiscoverReport) {
    ...
    // Group by strategy
    let mut by_strategy: std::collections::BTreeMap<
        &str,
        Vec<&crate::code::c4_discover::Container>,
    ...
    println!(
        "Container candidates ({} strategies, {} candidates):\n",
        by_strategy.len(),
        total
    );
...
pub fn print_call_graph_table(report: &CallGraphReport) {
    ...
    for node in &report.nodes {
        by_lang.entry(node.language).or_default().push(node);
    }
    ...
    for (lang, nodes) in &by_lang {
        ...
        for node in &**nodes {
            ...
            println!(
                "    ✓ {}  {}:{}  (conf {:.2})",
                fq, node.file, node.line, node.confidence
            );
        }
    }
...
pub fn print_class_diagram_table(report: &ClassDiagramReport) {
    ...
    if !report.edges.is_empty() {
        ...
        for edge in &report.edges {
            ...
            println!(
                "    {} --{pred}--> {}  (conf {:.2})",
                src_name, tgt_name, edge.confidence
            );
        }
    }
...
pub fn print_sequence_table(report: &SequenceReport) {
    ...
    for i in &report.interactions {
        println!(
            "  {:3}. [d{}] {} → {} ({:?})",
            i.order_key, i.depth, i.sender, i.receiver, i.message_kind
        );
