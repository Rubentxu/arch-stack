...
mod tests {
    ...
    /// Green cell: PureDeterministic × Observed → "accepted" (per spec-35 v1.1 §6).
    #[test]
    ...
    /// ModelInference × Suggested: all ModelInference execution is blocked by the
    /// trust gate regardless of authority level → "drafted".
    #[test]
    fn recompute_status_blocks_model_inference_to_observed_with_trust_violation() {
        // ModelInference with a non-llm_analyst tool still gets ModelInference execution.
        // canonical_promotion_allowed(ModelInference, _) = Err.
    ...
    /// should_warn_pending_adjudication returns true for ModelInference × Suggested
    /// with Accept verdict — the m30 bridge must emit warn and set the flag.
    #[test]
    fn recompute_status_sets_pending_adjudication_event_on_feedback_accept_for_model_inference() {
        ...
        let should_warn = should_warn_pending_adjudication(trust, verdict);
        assert!(
            should_warn,
            "ModelInference × Suggested + Accept should trigger m30 bridge warning"
        );
        // Non-ModelInference should not warn
        ...
        assert!(
            ...
            "HumanDecision should not trigger pending adjudication"
        );
        // ModelInference + Reject should not warn (reject never silently promotes)
        assert!(
            !should_warn_pending_adjudication(trust, crate::feedback::FeedbackVerdict::Reject),
            "ModelInference + Reject should not trigger pending adjudication"
        );
    }
}
